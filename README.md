...
**Google Workspace MCP (`stdio`):** Google **Drive, Sheets, Docs, Gmail, Calendar**, etc. on the Neuuf coordinator are available through **`google_workspace_mcp_*`** only (user OAuth via **`npx google-workspace-mcp setup`**; see **`docs/INTEGRATIONS_WALKTHROUGH.md`** §2). **`iso-neuuf-coordinator`** sets **`ISO_AGENT_GOOGLE_WORKSPACE_MCP_TRANSPORT=stdio`** and **`ISO_AGENT_GOOGLE_WORKSPACE_MCP_DEBUG=true`** when neither the shell nor an uncommented **`.env`** (cwd) line sets them (MCP-related stderr only, not Bedrock/botocore); set **`disabled`** / **`false`** only if you intentionally want **no** Google file tools for that process.
