...
5. **Models** — Upstream Strands can attach different **model providers** (see strandsagents.com and the SDK). **This repository** standardizes on **Amazon Bedrock only**: `get_default_model()` builds Strands **`BedrockModel`**, so generations go through **Bedrock Runtime** (AWS credentials, IAM, regional model access)—**not** the separate Anthropic Cloud **Messages API**. You pick the **Bedrock foundation model or inference profile** (e.g. `ISO_AGENT_BEDROCK_MODEL_ID`); use whatever FMs your AWS account is entitled to invoke in that region, per AWS and Strands Bedrock docs.
