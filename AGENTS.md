...
| `src/iso_agent/l3_runtime/default_model.py` + `config.py` (`llm_provider`, `ISO_AGENT_BEDROCK_*`, Anthropic ids) | Default **Bedrock Runtime** (`BedrockModel`); optional Anthropic direct via `ISO_AGENT_LLM_PROVIDER=anthropic` + `ANTHROPIC_API_KEY` |
...
**Strands SDK context:** keep the upstream clone at **`references/STRANDS_SDK.md`** (local path + reading order) in scope—ideally open it in the **same Cursor workspace** (multi-root) as this repo when developing so searches and navigation include `src/strands/` best practices. For **AWS model wiring**, follow Strands’ documented **`BedrockModel`** path: [Amazon Bedrock model provider](https://strandsagents.com/docs/user-guide/concepts/model-providers/amazon-bedrock/) (also indexed in **`references/STRANDS_OFFICIAL_DOCS.md`**).
