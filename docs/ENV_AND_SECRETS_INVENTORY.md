...
**Convention:** `ISO_AGENT_*` variables are loaded by `Settings` (see `config.py`). A few vendors use **standard names** without that prefix (`NOTION_TOKEN` for manual REST only, `PERPLEXITY_API_KEY`).
...
| Path | Purpose |
|------|---------|
| `secrets/**/*.json` | Optional local JSON (e.g. OAuth client exports) — **gitignored**; do not commit |
...
Google (Drive, Sheets, Docs, etc.) on the Neuuf coordinator is **only** via **Google Workspace MCP** — see **`docs/INTEGRATIONS_WALKTHROUGH.md`** §2 and **`secrets/README.md`**.
