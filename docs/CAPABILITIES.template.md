...
- [x] **Phase 4 — Notion QMS (draft + read)** — `notion_create_qms_draft` / `notion_read_page`; `NOTION_TOKEN` + allowlisted page UUIDs; optional hosted **Notion MCP** (default `hybrid` transport, `notion_mcp_oauth_interactive_login` in REPL, `iso-notion-mcp-login`) — see `docs/NOTION_MCP.md`
